...
class HrpClusteringTests(unittest.TestCase):
    ...
    def test_hrp_rejects_invalid_price_history(self):
        invalid_prices = (
            pd.DataFrame(),
            pd.DataFrame({"BTC/USDT": [100.0, 101.0, 102.0]}),
            pd.DataFrame({"BTC/USDT": [100.0, 101.0], "ETH/USDT": [10.0, 11.0]}),
            pd.DataFrame({"BTC/USDT": [100.0, 101.0, 102.0], "ETH/USDT": [10.0, np.nan, 12.0]}),
        )
