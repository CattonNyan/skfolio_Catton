...
def print_monte_carlo_report(res: dict[str, object]):
    ...
    print(f"Expected Final Wealth    : ${res['expected_final_wealth']:,.2f}")
    print(f"Median Final Wealth      : ${res['median_final_wealth']:,.2f}")
    print(f"95% Confidence Band      : ${res['worst_case_5pct']:,.2f} ~ ${res['best_case_95pct']:,.2f}")
    print("--------------------------------------------------------------------------------")
    print("                      DOWNSIDE RISK & PROBABILITY PROFILE                       ")
    print("--------------------------------------------------------------------------------")
    print(f"Value at Risk (VaR 95%)  : ${res['var_95_dollar']:,.2f} (최대 5% 확률 손실액)")
    print(f"Value at Risk (VaR 99%)  : ${res['var_99_dollar']:,.2f} (최대 1% 극단 손실액)")
    print(f"Conditional VaR (CVaR 95%): ${res['cvar_95_dollar']:,.2f} (95% 초과 손실 발생 시 평균 손실액)")
    print(f"원금 손실 확률 (Prob Loss): {res['prob_loss_pct']:.2f}%")
    print(f"원금 30% 이상 폭락 확률  : {res['prob_severe_loss_pct']:.2f}%")
    print(f"원금 2배 달성 확률      : {res['prob_doubling_pct']:.2f}%")
    print("================================================================================\n")
